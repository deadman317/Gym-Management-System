...
import { handleLogout, getCurrentUser, fetchAccountDetails, fetchMemberDetails } from './auth.js';
    ...
    .then((user) => {
        if (user) {
            ...
            fetchMemberDetails(user)
                .then((memberData) => {
                    if (memberData) {
                        document.getElementById("planType").textContent = memberData.planType;
                        document.getElementById("planPrice").textContent = memberData.planPrice;
                        document.getElementById("dateOfSubscription").textContent = memberData.dateOfSubscription;
                        document.getElementById("subscriptionEndDate").textContent = memberData.subscriptionEndDate;
                    } else {
                        // Handle the case when member data is not available or not found
                        console.log("Member data not available.");
                    }
                })
                .catch((error) => {
